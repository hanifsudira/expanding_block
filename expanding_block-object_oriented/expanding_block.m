function [imgMasked, imgOut] = expanding_block(imgIn, varargin)
...
S = 1:ceil(log2(init.blockSize));   
S = 2.^S;
S(end) = blockSize;

parfor n=1:numel(buckets)
    for m = 1:numel(S)
        bucket{n} = process_bucket(bucket{n}, S(m))
    end
end
...
%% 11. OUTPUT
[imgMasked, imgOut] = write_masked(mask, img_gray_full);       
